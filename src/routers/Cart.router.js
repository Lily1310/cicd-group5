...
router.post('/checkout/:personId', async (req, res) => {
  const personId = parseInt(req.params.personId, 10);
  const { selectedItemsInt } = req.body;
  ...
  try {
    ...
    const cartItems = await prisma.cartitem.findMany({
      ...
      console.error('Error in findMany query:', error);
    });
    ...
    if (insufficientStockItems.length > 0) {
      const insufficientStockDetails = insufficientStockItems.map(item => ({
        productName: item.product.name,
        requested: item.quantity,
        available: item.product.stockQuantity,
      }));

      console.log(insufficientStockDetails)

      return res.status(400).json({
        error: 'Some items do not have enough stock.',
        insufficientItems: insufficientStockDetails,
      });
    }
